...
def inicio():
    ...
    evaluaciones_validas = [
        ...
        if e.nivel in VALORES_ILUO
    ]

    # ========================================================
    # CALCULO GENERAL ILUO
    # ========================================================
...
# ========================================================
    ...
    consulta_departamentos = Departamento.query

    if area_id:
        consulta_departamentos = consulta_departamentos.filter_by(
            area_id=area_id
        )

    departamentos = consulta_departamentos.order_by(
        Departamento.nombre
    ).all()

    # ========================================================
    # DETERMINAR DEPARTAMENTOS A UTILIZAR
    # ========================================================
    ...
    if departamento_id:
        ...
        departamento_seleccionado = Departamento.query.get(
            departamento_id
        )

        if departamento_seleccionado:

            if not area_id or departamento_seleccionado.area_id == area_id:
                departamentos_ids = [
                    departamento_id
                ]
    ...
    colaboradores_consulta = Colaborador.query.filter(
        Colaborador.activo == True
    )

    if departamentos_ids:

        colaboradores_consulta = colaboradores_consulta.filter(
            Colaborador.departamento_id.in_(departamentos_ids)
        )
    ...
    colaboradores_dashboard = colaboradores_consulta.all()

    colaborador_ids = [
        c.id
        for c in colaboradores_dashboard
    ]
    ...
    if colaborador_ids:
        ...
        evaluaciones_dashboard = Evaluacion.query.filter(
            Evaluacion.colaborador_id.in_(colaborador_ids)
        ).all()

    else:

        evaluaciones_dashboard = []
    ...
    valores_validos = [
        valores_iluo[e.nivel]
        for e in evaluaciones_dashboard
        if e.nivel in valores_iluo
    ]
    ...
    estado_general = obtener_estado_dominio(
        porcentaje_general
    )
    ...
    for evaluacion in evaluaciones_dashboard:

        if evaluacion.nivel in niveles_general:
    ...
    total_evaluaciones = len(evaluaciones_dashboard)

    distribucion_iluo = []

    for nivel in ["I", "L", "U", "O"]:

        cantidad = niveles_general[nivel]
    ...
    for evaluacion in evaluaciones_dashboard:
        ...
        if evaluacion.nivel not in valores_iluo:
            continue

        habilidad = Habilidad.query.get(
        ...
        if habilidad_id not in habilidades_mapa:

            habilidades_mapa[habilidad_id] = {
                "nombre": habilidad.nombre,
                "total": 0,
                "bajas": 0,
                "altas": 0,
        ...
        registro = habilidades_mapa[
            habilidad_id
        ]

        registro["total"] += 1
        registro["suma"] += valores_iluo[
            evaluacion.nivel
        ]

        # I y L representan necesidad de desarrollo
        if evaluacion.nivel in ["I", "L"]:

            registro["bajas"] += 1

        # U y O representan dominio consolidado
    ...
    for registro in habilidades_mapa.values():
    ...
    for departamento in departamentos:
        ...
        colaboradores_departamento = [
            c
            for c in colaboradores_dashboard
            if c.departamento_id == departamento.id
        ]

        ids_departamento = [
            c.id
            for c in colaboradores_departamento
        ]

        evaluaciones_departamento = [
            e
            for e in evaluaciones_dashboard
            if e.colaborador_id in ids_departamento
        ]

        valores_departamento = [
            valores_iluo[e.nivel]
            for e in evaluaciones_departamento
            if e.nivel in valores_iluo
        ]

        if valores_departamento:

            dominio_departamento = round(
                sum(valores_departamento) /
                len(valores_departamento),
                1
            )

        else:

            dominio_departamento = 0

        estado_departamento = obtener_estado_dominio(
        ...
        resumen_departamentos.append({
            ...
            "dominio": dominio_departamento,
            "estado": estado_departamento["estado"],
            "color": estado_departamento["color"]
        })
    ...
    if area_id:
        ...
        area_seleccionada = Area.query.get(
            area_id
        )

        if area_seleccionada:

            nombre_area = area_seleccionada.nombre
    ...
    if departamento_id:
        ...
        departamento_actual = Departamento.query.get(
            departamento_id
        )
        ...
        if departamento_actual:
            ...
            titulo_filtro = (
                nombre_area
                + " – "
                + departamento_actual.nombre
            )

        else:

            titulo_filtro = nombre_area
    ...
    return render_template(
        ...
        colaboradores=colaboradores_dashboard,
        ...
        niveles_general=niveles_general,
        distribucion_iluo=distribucion_iluo,
