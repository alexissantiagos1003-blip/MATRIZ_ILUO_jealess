...
def inicio():
    ...
    if area_id:
        ...
        area_actual = Area.query.get(
            area_id
        )

    else:

        area_actual = areas[0] if areas else None

        if area_actual:
            area_id = area_actual.id

    # ========================================================
    # DEPARTAMENTOS DEL AREA
    # ========================================================
    ...
    if departamento_id:
        ...
        if departamento_actual:
            ...
            if (
                not area_actual
                or departamento_actual.area_id != area_actual.id
            ):
    ...
    consulta_colaboradores = Colaborador.query.filter_by(
        activo=True
    )

    if departamento_actual:

        consulta_colaboradores = consulta_colaboradores.filter_by(
            departamento_id=departamento_actual.id
        )
        ...
        if departamento_ids:
            ...
            consulta_colaboradores = consulta_colaboradores.filter(
                Colaborador.departamento_id.in_(
                    departamento_ids
                )
            )

    colaboradores_dashboard = consulta_colaboradores.all()
    ...
    # ========================================================
    # EVALUACIONES DEL DASHBOARD
    # ========================================================

    evaluaciones_dashboard = Evaluacion.query.filter(
        Evaluacion.colaborador_id.in_(
            colaborador_ids
        )
    ).all() if colaborador_ids else []

    # ========================================================
    # CALCULO DE DOMINIO
    # ========================================================

    dominio = calcular_dominio_evaluaciones(
