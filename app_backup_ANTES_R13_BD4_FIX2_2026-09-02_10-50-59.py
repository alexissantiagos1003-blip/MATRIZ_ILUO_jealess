...
@app.route("/roles-permisos", methods=["GET", "POST"])
@login_required
...
def roles_permisos():
    ...
    def categoria_visible(rol_nombre, puesto_nombre):

        nombre_rol = (rol_nombre or "").strip().lower()
        nombre_puesto = (puesto_nombre or "").strip().lower()

        if nombre_puesto == "gerente general":
            return "Gerente General"

        if nombre_rol == "gerente_area":
    ...
    if request.method == "POST":
        ...
        try:
            ...
            rol_nombre = mapa_roles.get(
                categoria
            )

            if not rol_nombre:
                raise ValueError(
                    "Rol de interfaz no válido."
                )
            ...
            for permiso in permisos:
                ...
                campo_permitido = (
                    f"permiso_{permiso.id}"
                )

                campo_alcance = (
                    f"alcance_{permiso.id}"
                )
    ...
    if lider is not None and supervisor is not None:
    ...
    for relacion in relaciones_puesto_rol:
    ...
    for rol in roles:
        ...
        for rp in asignaciones:
            ...
            rol_permiso[rol.id][rp.permiso_id] = {
                "permitido": bool(
                    rp.permitido
                ),
                "alcance": (
