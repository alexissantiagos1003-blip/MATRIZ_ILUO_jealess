...
def pdf_evaluacion(evaluacion_id):
    ...
    filas = [
        [
            ...
            Paragraph("<b>Comentario</b>", normal)
        ]
    ]
...
# =========================
# VER EVALUACIÓN POR FOLIO
# =========================

@app.route("/evaluacion/ver/<folio>")
def ver_evaluacion(folio):
    ...
    colaborador = evaluaciones[0].colaborador
...
@app.route("/evaluacion/pdf/<folio>")
def pdf_evaluacion(folio):
    ...
    datos_principales = [
        ...
        [
            "Colaborador",
            colaborador.nombre if colaborador else "—"
        ],
        [
            "Evaluador",
            evaluaciones[0].evaluador or "—"
        ],
        [
            "Rol del evaluador",
            evaluaciones[0].rol_evaluador or "—"
        ],
        [
            "Fecha",
            str(evaluaciones[0].fecha_evaluacion)
            if evaluaciones[0].fecha_evaluacion
            else "—"
        ],
    ]
    ...
    for evaluacion in evaluaciones:
        ...
        habilidad = (
            evaluacion.habilidad.nombre
            if evaluacion.habilidad
            else "—"
        )
    ...
    comentario = evaluaciones[0].comentario or "Sin comentarios."
    ...
    confirmada = (
        "Sí"
        if evaluaciones[0].confirmacion_colaborador
        else "No"
    )

    firma = (
        "Firma registrada"
        if evaluaciones[0].firma_archivo
        else "Sin firma registrada"
    )

    datos_validacion = [
        ["Confirmación del colaborador", confirmada],
        ["Firma", firma],
    ]
    ...
    elementos.append(
        Paragraph(
            "Documento generado por el Sistema de Matriz ILUO.",
            subtitulo
        )
    )
